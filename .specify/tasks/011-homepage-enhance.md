...
- [x] 输入 1 字符后开始本地搜索
- [x] 搜索下拉底部显示 "按 Enter 问 AI 更多" 入口
- [x] 进入 Chat 模式后显示对话气泡，支持 SSE 流式（mock 模式）

### 拼写纠错（模糊匹配）
- [x] Levenshtein 编辑距离算法，距离 ≤ 2 时触发
- [x] 搜索无结果时显示 "🤔 您是不是要找: XXX"
- [x] 纠错建议展示完整帕鲁名 + 属性 + 稀有度星级
- [x] 点击纠错建议直接搜索正确帕鲁名

### 快捷功能入口
- [x] 桌面端 3 列网格，移动端 2 列网格
- [x] 6 个入口：繁殖计算器 / 属性克制 / 地图 / AI 助手 / Tier 排行 / 物品图鉴
- [x] 每个入口有图标 + 文字 + 点击跳转

### 热门帕鲁
- [x] 桌面端 4-8 张网格展示
- [x] 移动端横向滑动（overflow-x-auto + snap-x）
- [x] 展示帕鲁名 + 元素属性 + 稀有度
- [x] 数据从 pals.json 按稀有度排序取前 8

### 响应式
- [x] 桌面：搜索居中 → 快捷入口(3列) → 热门帕鲁(网格)
- [x] 移动端：搜索顶部 → Chat → 快捷入口(2列) → 热门帕鲁(横滑)
